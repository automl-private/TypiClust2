...
def get_train_dataset(p, transform, to_augmented_dataset=False,
                        to_neighbors_dataset=False, split=None, root='.', download=False):
    # Base dataset
...
def get_val_dataset(p, transform=None, to_neighbors_dataset=False, root='.', download=False):
    # Base dataset
    if p['val_db_name'] == 'cifar-10':
        from data.cifar import CIFAR10
        dataset = CIFAR10(root=root, train=False, transform=transform, download=download)

    elif p['val_db_name'] == 'cifar-100':
        from data.cifar import CIFAR100
        dataset = CIFAR100(root=root, train=False, transform=transform, download=download)

    elif p['val_db_name'] == 'stl-10':
        from data.stl import STL10
        dataset = STL10(root=root, split='test', transform=transform, download=download)

    elif p['train_db_name'] == 'tiny-imagenet':
        from data.tinyimagenet import TinyImageNet
        dataset = TinyImageNet(root=root, split='val', transform=transform)
